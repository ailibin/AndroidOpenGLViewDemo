...
//        this(0, 0, 20, 0, 0, -1, 0, 1, 0);
    ...
    public synchronized float getAngle(float dX, float dY) {
        if (dX == 0 && dY == 0) {
            return 0;
        }
        float vlen;
    ...
    public float[] translateCameraAngleImpl(float dX, float dY) {
        float vlen;
        // Translating the camera requires a directional vector to rotate
        // First we need to get the direction at which we are looking.
        // The look direction is the view minus the position (where we are).
        ...
        if (dX != 0 && dY != 0) {
            ...
            // in this case we use the vlen angle because the diagonal is not perpendicular
            // to the initial Right and Arriba vectors
            return createRotationMatrixAroundVector(buffer, 24, vlen, rotX, rotY, rotZ, true);
        } else if (dX != 0) {
            // in this case the user is drawing an horizontal line: <-- ó -->
            return createRotationMatrixAroundVector(buffer, 24, dX, xArriba, yArriba, zArriba, true);
        } else {
            // in this case the user is drawing a vertical line: |^  v|
            return createRotationMatrixAroundVector(buffer, 24, dY, xRight, yRight, zRight, true);
        }
    ...
    public static void createRotationMatrixAroundVector(float[] matrix, int offset, float angle, float x, float y,
        ...
        Matrix.rotateM(matrix, offset, angle, x, y, z);


    }

    public static float[] createRotationMatrixAroundVector(float[] matrix, int offset, float angle, float x, float y,
                                                           float z, boolean isReturn) {
        float cos = (float) Math.cos(angle);
        float sin = (float) Math.sin(angle);
        float cos_1 = 1 - cos;
