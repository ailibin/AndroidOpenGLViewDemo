...
public final class Object3DBuilder {
    ...
    public static Object3DData buildPoint(float[] point) {
        return new Object3DData(createNativeByteBuffer(point.length * 4).asFloatBuffer().put(point))
                .setDrawMode(GLES20.GL_POINTS).setId("Point").setVisible(false);
    }
    ...
    public static Object3DData loadV5( Uri modelUri) {
        try {
            InputStream is = new URL(modelUri.toString()).openStream();
            WavefrontLoader wfl = new WavefrontLoader(modelUri.toString());
            wfl.analyzeModel(is);
    ...
    public static Object3DData generateArrays(Object3DData obj) throws IOException {
        ...
        if (faces == null) {
            Log.e(TAG, "No faces. Not generating arrays");
            return obj;
        }

        Log.e(TAG, "Allocating vertex array buffer... Vertices (" + faces.getVerticesReferencesCount() + ")");
        ...
        Log.e(TAG, "Populating vertex array...");
        ...
        Log.e(TAG, "Allocating vertex normals buffer... Total normals (" + faces.facesNormIdxs.size() + ")");
        ...
        if (vertexNormalsBuffer != null && vertexNormalsBuffer.capacity() > 0) {
            Log.e(TAG, "Populating normals buffer...");
            ...
            Log.e(TAG, "Model without normals. Calculating [" + faces.getIndexBuffer().capacity() / 3 + "] normals...");
        ...
        if (materials != null) {
            ...
            try (InputStream inputStream = ContentUtils.getInputStream(materials.mfnm)) {
                ...
                Log.e(TAG, "Couldn't load material file " + materials.mfnm + ". " + ex.getMessage(), ex);
                obj.addError(materials.mfnm + ":" + ex.getMessage());
            }
        }

        if (materials != null && !faceMats.isEmpty()) {
            Log.e(TAG, "Processing face materials...");
        ...
        if (materials != null && !materials.materials.isEmpty()) {
            ...
            if (texture != null) {
                Log.e(TAG, "Loading texture " + texture);
                ...
                Log.e(TAG, "Found material(s) but no texture");
            }
        } else {
            Log.e(TAG, "No materials -> No texture");
        }
        ...
        if (texCoords != null && texCoords.size() > 0) {
            ...
            Log.e(TAG, "Allocating/populating texture buffer (flipTexCoord:" + obj.isFlipTextCoords() + ")...");
            ...
            Log.e(TAG, "Populating texture array buffer...");
            ...
            try {
                    ...
                    for (int j = 0; j < text.length; j++) {
                        if (textureData == null || textureOk) {
                            if (text[j] * 2 >= 0 && text[j] * 2 < textureCoordsBuffer.limit()) {
                                ...
                                Log.e(TAG, "Wrong texture for face " + i);
                ...
                if (!anyTextureOk) {
                    Log.e(TAG, "Texture is wrong. Applying global texture");
                    counter = 0;
                ...
                Log.e(TAG, "Failure to load texture coordinates", ex);
            }
        }
        //}
    ...
    public static Object3DData buildFaceNormals(Object3DData obj) {
        ...
        if (vertexBuffer == null) {
            Log.e(TAG, "Generating face normals for '" + obj.getId() + "' I found that there is no vertex data");
            return null;
        }
        ...
        if (drawBuffer != null) {
            Log.e(TAG, "Generating face normals for '" + obj.getId() + "' using indices...");
            ...
            if (vertexBuffer.capacity() % (/* COORDS_PER_VERTEX */3 * /* VERTEX_PER_FACE */ 3) != 0) {
                ...
                Log.e(TAG, "Generating face normals for '" + obj.getId()
    ...
    public static AnimatedModel buildSkeleton(AnimatedModel animatedModel) {
        ...
        Log.e(TAG, "Building " + skeleton.getJointCount() + " bones...");
